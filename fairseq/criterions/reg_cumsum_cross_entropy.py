...
@register_criterion('reg_cumsum_cross_entropy')
class RegCumSumCrossEntropyCriterion(FairseqCriterion):
    ...
    def compute_cumsum_regularization(self, lprobs, target):
        neg_lprobs = lprobs * -1
        ...
        for t_i in range(0, target.size()[1]):
            regs[:, t_i] = torch.sum(max_probs[:, :t_i+1], -1)
        return regs

    def compute_loss(self, model, net_output, sample, reduce=True):
        lprobs = model.get_normalized_probs(net_output, log_probs=True)
        target = model.get_targets(sample, net_output)
        regs = self.compute_cumsum_regularization(lprobs, target)

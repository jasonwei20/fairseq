...
@register_criterion('reg_stdev_cross_entropy')
class RegStdevCrossEntropyCriterion(FairseqCriterion):
    ...
    def compute_stdev_regularization(self, lprobs, target):
        neg_lprobs = lprobs * -1
        ...
        for t_i in range(1, target.size()[1]):
            regs[:, t_i] = torch.std(gt_probs_reduced[:, :t_i+1], dim=-1)
        return regs

    def compute_loss(self, model, net_output, sample, reduce=True):
        lprobs = model.get_normalized_probs(net_output, log_probs=True)
        target = model.get_targets(sample, net_output)
        regs = self.compute_stdev_regularization(lprobs, target)
